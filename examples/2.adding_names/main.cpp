...
int main()
{
	...
	Float squaredDistance = dot(fragmentToLight, fragmentToLight).SetName("squaredDistance");

	vec3 normalized_fragmentToLight = fragmentToLight / sqrt(squaredDistance);

	Float NdotL = dot(normal, normalized_fragmentToLight).SetName("NdotL");

	vec3 DiffuseTerm = (max(NdotL, 0.0) * DirectLightColor / squaredDistance).SetName("DiffuseTerm");
